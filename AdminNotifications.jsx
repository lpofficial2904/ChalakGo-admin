...
export default function AdminNotifications({ onOpen }) {
      ...
      try {
        ...
        if (fresh.length) {
          ...
          if (audio.current?.state === 'running') {
            ...
            oscillator.start()
            oscillator.stop(audio.current.currentTime + 0.4)
          }
  ...
  const enableAlerts = async () => {
    try {
      ...
      toast.info('Alerts enabled while this admin panel is open. Browser alerts depend on browser permission.')
  ...
  return <div className="admin-notifications">
    ...
    {open && <section id="admin-notification-list" className="notification-panel" aria-label="Notifications" onKeyDown={event => { if (event.key === 'Escape') setOpen(false) }}>
      ...
      <div className="notification-actions">
        ...
        <button onClick={enableAlerts}>{sound ? 'Sound enabled / browser alerts' : 'Enable sound & browser alerts'}</button>
      </div>
      <small>Checks every 5 seconds while the panel is open.</small>
